...
func HandlePDUSessionResourceReleaseResponse(ctx ctxt.Context, ran *context.AmfRan, message *ngapType.NGAPPDU) {
	...
	if pDUSessionResourceReleasedList != nil {
		...
		for _, item := range pDUSessionResourceReleasedList.List {
			...
			} else if responseErr != nil && responseErr.JsonData != nil && responseErr.JsonData.Error.Cause != nil {
				ranUe.Log.Errorf("SendUpdateSmContextN2Info[PDUSessionResourceReleaseResponse] Error: %+v",
					responseErr.JsonData.Error.GetCause())
...
func HandlePDUSessionResourceNotify(ctx ctxt.Context, ran *context.AmfRan, message *ngapType.NGAPPDU) {
	...
	for _, item := range pDUSessionResourceNotifyList.List {
		...
		if response != nil {
			...
			if n2Info != nil {
				switch responseData.GetN2SmInfoType() {
	...
	for _, item := range pduSessionResourceModifyIndicationList.List {
		...
		if errResponse != nil && errResponse.BinaryDataN2SmInformation != nil {
			...
			ngap_message.AppendPDUSessionResourceFailedToModifyListModCfm(&pduSessionResourceFailedToModifyListModCfm,
				int64(pduSessionID), binaryDataN2SmInformation)
		}
	}
...
func HandlePathSwitchRequest(ctx ctxt.Context, ran *context.AmfRan, message *ngapType.NGAPPDU) {
	...
	if uESecurityCapabilities != nil {
		// UESecurityCapabilities has CRITICALITY ignore in PathSwitchRequest
		// (TS 38.413, PathSwitchRequestIEs). BIT STRING (SIZE(16,...)) must be
		// at least 2 bytes; when the APER extension bit is set the decoder may
		// return fewer bytes. Guard against that to avoid an index panic.
		if len(uESecurityCapabilities.NRencryptionAlgorithms.Value.Bytes) < 2 ||
			len(uESecurityCapabilities.NRintegrityProtectionAlgorithms.Value.Bytes) < 2 {
			ran.Log.Warnln("UESecurityCapabilities bit string too short in PathSwitchRequest; ignoring IE per TS 38.413 PathSwitchRequestIEs criticality IGNORE")
		} else {
	...
	if pduSessionResourceToBeSwitchedInDLList != nil {
		for _, item := range pduSessionResourceToBeSwitchedInDLList.List {
			...
			if response != nil && response.GetBinaryDataN2SmInformation() != nil {
				...
				pduSessionResourceSwitchedItem.PathSwitchRequestAcknowledgeTransfer = binaryDataN2SmInformation
...
func HandleHandoverRequired(ctx ctxt.Context, ran *context.AmfRan, message *ngapType.NGAPPDU) {
	...
	if !ok {
		...
		ranNodeId := models.NewNullableGlobalRanNodeId(&targetRanNodeId)
